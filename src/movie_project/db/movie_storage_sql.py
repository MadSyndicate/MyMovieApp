...
def list_movies():
    ...
    with engine.connect() as connection:
        result = connection.execute(text(
            """SELECT title, year, rating, poster_url, user_notes, imdb_id FROM movies
    ...
    with engine.connect() as connection:
        try:
            connection.execute(text(
                """INSERT INTO movies (title, year, rating, poster_url, user_id, imdb_id) 
                   VALUES (:title, :year, :rating, :poster, :user_id, :imdb_id)"""),
               {
                   "title": title,
                   "year": year,
                   "rating": rating,
...
def update_movie_notes(title, notes):
    ...
    with engine.connect() as connection:
        try:
            connection.execute(text(
                """UPDATE movies SET user_notes = :notes 
